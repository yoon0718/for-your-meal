...
export default function AudioRecorder() {
  ...
  const handleStartRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        setRecording(false);
        setDisplayText(''); // 녹음이 종료되면 텍스트를 비웁니다.
            setTextVisible(false); // 텍스트를 숨깁니다.
      };
  ...
  return (
    <div>
      ...
      {textVisible && ( // textVisible 상태가 true일 때만 아래 내용을 렌더링
        <p
          className={`ptag ${textVisible ? 'fade-in' : 'fade-out'}`}
